...
| 变量 | 来源 |
| --- | --- |
| `HERMES_HOME` | 用户传入值；默认 Windows 为 `C:\ProgramData\SSC\ZhanClaw\Hermes`，Unix 为 `$HOME/.hermes` |
...
- Hermes 配置：Windows 默认为 `C:\ProgramData\SSC\ZhanClaw\Hermes\config.yaml` 和 `C:\ProgramData\SSC\ZhanClaw\Hermes\.env`；Unix 默认为 `~/.hermes/config.yaml` 和 `~/.hermes/.env`；可通过 `HERMES_HOME` 覆盖。
